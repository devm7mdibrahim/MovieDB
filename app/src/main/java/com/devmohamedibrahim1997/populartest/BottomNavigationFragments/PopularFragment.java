...
public class PopularFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        FragmentPopularBinding popularBinding = DataBindingUtil.inflate(inflater,R.layout.fragment_popular, container, false);
        movieRecyclerView = popularBinding.popularRecyclerView;
    ...
    private void onMovieClick() {
        recyclerViewAdapter.setOnItemClickListener((position, v) -> {
            ...
            if (movieViewModel.getMovies("popular").getValue() != null) {
                intent.putExtra("movieId", movieViewModel.getMovies("Popular").getValue().get(position).getId());
            }
