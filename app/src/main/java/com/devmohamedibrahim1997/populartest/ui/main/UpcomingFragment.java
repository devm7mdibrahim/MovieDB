...
import static com.devmohamedibrahim1997.populartest.utils.Constant.UPCOMING_QUERY;
...
public class UpcomingFragment extends Fragment {
    ...
    private void getMovies() {
        if (isNetworkAvailable(activity)) {
            movieViewModel.getMovies(UPCOMING_QUERY).observe(this, movies -> {
                if (movies != null) {
    ...
    private void onMovieClick() {
        recyclerViewAdapter.setOnItemClickListener((position, v) -> {
            if (movieViewModel.getMovies(UPCOMING_QUERY).getValue() != null) {
                Intent intent = new Intent(activity, DetailsActivity.class);
                intent.putExtra(MOVIE_ID, movieViewModel.getMovies(UPCOMING_QUERY).getValue().get(position).getId());
    ...
    private void initRecyclerView() {
        ...
        RecyclerView movieRecyclerView = upcomingBinding.upcomingRecyclerView;
